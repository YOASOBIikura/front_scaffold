...
const props=defineProps({
    ...
    decimalsShow: {type: Number, default: 6, require: true},
    symbol:{
        type:String,
        default:""
    },
...
onMounted(()=>{
    if(BigNumber.from(props.value)){
        data.inputShow=props.value.div(ethers.utils.parseUnits("1",props.decimals-props.decimalsShow)).toNumber()/10 ** props.decimalsShow
    }
})
watch(
    () => props.valueChange,
    (val) => {
        data.inputShow = val.div(ethers.utils.parseUnits("1",props.decimals-props.decimalsShow)).toNumber()/10 ** props.decimalsShow;
...
var max=()=>{
    ...
    data.inputShow=String(BigNumber.from(value).div(ethers.utils.parseUnits("1",props.decimals-props.decimalsShow)).toNumber()/10 ** props.decimalsShow)
...
var inputValue= (input)=>{
    ...
    if(props.isMax && BigNumber.from("0").eq(BigNumber.from(props.maxValue))){
        ...
        if(value!=""){
            ...
            if(props.isMax  && !BigNumber.from(props.maxValue).eq(BigNumber.from("0"))  && value.mul(ethers.utils.parseUnits("1",3)).div(props.maxValue).gte(BigNumber.from("999"))  && String(data.preValue).length<=String(data.inputShow).length){
                value=props.maxValue    
                //触发最大值的情况下修改显示值
                nextTick(() => {
                    data.inputShow=props.maxValue.div(ethers.utils.parseUnits("1",props.decimals-props.decimalsShow)).toNumber()/10 ** props.decimalsShow
                })
            } 
        }else{
            value=BigNumber.from("0")
    ...
    //最大值为0 并且要最大值时
    if(props.isMax && BigNumber.from(props.maxValue).eq(BigNumber.from("0"))){
        value=props.maxValue   
        nextTick(()=>{
            data.inputShow="0"
        })
...
var blurChange=()=>{  
   ...
   let newValue=ethers.utils.parseUnits(String(value),props.decimals)
    ...
    if(props.isMax && !BigNumber.from(props.maxValue).eq(BigNumber.from("0")) && newValue.mul(ethers.utils.parseUnits("1",3)).div(props.maxValue).gte(BigNumber.from("999"))  && String(data.preValue).length<=String(data.inputShow).length){
